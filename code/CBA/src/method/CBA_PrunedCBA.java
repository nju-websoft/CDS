...
public class CBA_PrunedCBA {
    ...
    public static void readQuery(String directory, WeightedGraph ww) throws IOException
    {
        ...
        while((s=inputK.readLine())!=null)
        {
            ...
            for(String kw:temp)
            {
                if(ww.kwId.containsKey(kw.toLowerCase()))
                {
                    oneQuery.add(kw.toLowerCase());
                }
                else
                {
                    System.out.println(query.size()+" "+kw.toLowerCase());
                }
            }
            query.add(oneQuery);
    ...
    static void run(String directory, String weightingScheme, String method)
    {
        ...
        {
            for(int diameter=4;diameter<=4;diameter+=1)
            {
                ...
                totaltime[locOfQuery][diameter / 2 - 1] = endTime - startTime;
                // if (ans == null) {
                //     System.out.println("Error: ans is null");
                // }
                if(ansWeight!=null)
                {
